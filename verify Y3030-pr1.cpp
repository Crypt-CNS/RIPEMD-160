...
int main(){
	...
	while (test--){
        ...
        do{
			...
			T3 = LL(Y25, 10)+ ((Y28 &_and) |0x1000) + M + constant;
        }while((getNumber(T3, 10) != 0) || (getNumber(T3, 11) != 1));


        Y30 = LL(Y26, 10) + LL(T3, 15);
        unsigned int bit30 = getNumber(Y30, 30);

        T = LL(Y25,10) + F4(Y29, Y28, LL(Y27, 10)) + M + constant;
        Y30 = LL(Y26, 10) + LL(T, 15);

        if (bit30 == getNumber(Y30, 30)){
            count++;
        }
        else{
